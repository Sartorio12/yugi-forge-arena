-- 1. Drop ALL versions of the function to be safe
...
-- 2. Create the function with SIMPLE parameter names (best practice for PostgREST)
CREATE OR REPLACE FUNCTION public.admin_add_participant(tournament_id bigint, user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    IF NOT public.is_admin(auth.uid()) AND NOT EXISTS (
        SELECT 1 FROM public.tournaments WHERE id = admin_add_participant.tournament_id AND organizer_id = auth.uid()
    ) THEN
    ...
    IF EXISTS (
        ...
        WHERE tournament_id = admin_add_participant.tournament_id AND user_id = admin_add_participant.user_id
    ) THEN
    ...
    WHERE user_id = admin_add_participant.user_id;
    ...
    INSERT INTO public.tournament_participants (
        ...
        admin_add_participant.tournament_id,
        admin_add_participant.user_id,
        v_clan_id,
        false
    );
END;
