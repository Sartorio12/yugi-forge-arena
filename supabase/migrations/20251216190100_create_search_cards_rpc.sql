...
BEGIN
    ...
    WHERE
        (p_search_query IS NULL OR c.name ILIKE '%' || p_search_query || '%' OR c.pt_name ILIKE '%' || p_search_query || '%')
    AND
    ...
    AND
        (
            ...
            (p_genesys_points_operator = 'lte' AND c.genesys_points <= p_genesys_points_value)
        )
    ORDER BY
        CASE
