...
*   **News and Blog System:**
    ...
    *   **Storage RLS Hotfix (Feb 16, 2026):**
        *   Consolidated and fixed RLS policies for the `tournament_banners` bucket.
        *   Enabled full access for `super-admin`, `admin`, and `organizer` roles.
        *   Fixed permission for regular users to upload/manage their own banners in the subfolders.
        *   Ensured the bucket is explicitly public for optimized asset loading.
...
Para evitar conflitos de aspas entre o PowerShell (Windows) e o Bash (Linux), utilize **sempre** o método de Piping. Nunca tente passar a query como argumento direto do comando ssh.

**Padrão de Execução:**
```bash
"SUA QUERY SQL AQUI;" | ssh sart@deathstar-server "docker exec -i supabase-db psql -U postgres -d postgres"
```

### 2. Fluxo de Git (PowerShell)
O terminal PowerShell não aceita o operador `&&`. Os comandos devem ser executados de forma sequencial usando `;` ou em linhas separadas. Sempre realize o push imediatamente após o commit para manter o ambiente sincronizado.
...
- **Localização:** Todas as migrações devem ser criadas em `yugi-forge-arena/supabase/migrations/` com timestamp correto.
- **Execução:** Após criar o arquivo, ele deve ser aplicado no servidor usando o padrão de Piping do item 1:
  `cat caminho/da/migracao.sql | ssh sart@deathstar-server "docker exec -i supabase-db psql -U postgres -d postgres"`

### 4. Segurança e Robustez
- **Cargos:** Sempre incluir `super-admin` em verificações de permissão (RPCs e RLS).
- **Hardcoding:** O ID `80193776-6790-457c-906d-ed45ea16df9f` tem imunidade diplomática em rotas de admin para evitar race conditions.
...
*   **Local Database Migration (Feb 14, 2026):**
    ...
    *   **Automated Backup System (Feb 16, 2026):**
        *   A backup script has been configured on the server to perform full database dumps.
        *   **Location:** `/srv/samba/privado/yugi-forge-arena/backups`
        *   **Script:** `/srv/samba/privado/yugi-forge-arena/scripts/backup_db.sh`
        *   **Frequency:** Every 2 days at 03:00 AM (via crontab).
        *   **Retention:** Keeps the last 14 days of backups (compressed .sql.gz format).
